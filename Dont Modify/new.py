...
class WayPointServer(Node):
    ...
    def compute_pid(axis, desired, current, prev_err, err_sum, diff_prev,self):
            prev_err =  prev_err
            diff_prev =  diff_prev
            ...
            diff_filtered = self.beta * diff_prev + (1.0 - self.beta) * diff_raw

            # Integral handling (for axis 2 we may freeze externally)
            # For normal axes, accumulation happens here
            if axis != 2:
    ...
    out_x, ex, self.error_sum[0], self.diff_x = compute_pid(
        ...
        # --- Compute roll (Y) - reversed logic kept (works for your frame)
    out_y, ey, self.error_sum[1], self.diff_y = compute_pid(
            1, self.current_state[1], self.desired_state[1],  # intentional reversal kept
    ...
    out_way,eyaw,self.error_sum[3],self.diff_yaw = compute_pid(
            3, self.desired_state[3], self.yaw,
