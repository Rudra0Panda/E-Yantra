...
class WayPointClient(Node):
    ...
    def __init__(self):
        ...
        # For publishing current desired waypoint (optional)
        self.desired_pub = self.create_publisher(Vector3, '/desired_state', 10)

    # -------- Timer callback (runs at 5 Hz) -------- #
    def timer_callback(self):
        """Periodic loop running at 5 Hz for smooth control/monitoring."""
        if self.goal_index < len(self.goals):
            ...
            self.get_logger().debug(
                f'Publishing desired_state: X={desired_msg.x:.2f}, '
                f'Y={desired_msg.y:.2f}, Z={desired_msg.z:.2f}'
            )
        else:
            # Stop the timer once all goals done
    ...
    def get_result_callback(self, future):
        ...
        self.get_logger().info(f'Result: Hover time = {int(result.hover_time):.2f}s')
    ...
    def feedback_callback(self, feedback_msg):
        ...
        # Print feedback at limited rate
        if time.time() - self.last_feedback_time > 1.0 / self.rate_hz:
            self.get_logger().info(f'📡 Feedback: X={x:.2f}, Y={y:.2f}, Z={z:.2f}')
